...
getFileStats <- function(filepath) {    
    ...
    while (TRUE) {
        ...
        lineLengths = c(lineLengths, unlist(lapply(lines, function(line) { nchar(stringi::stri_trans_general(line, "latin-ascii"))})))    
...
saveRDS(object = stats, file = paste0(g_corpus_directory_en, '\\en_US.filestats.rds'))
htmlTable(stats, 
          rnames = FALSE,
...
names(sampleStats) = header = c('Sample Document', 'Lines Count', 'Words Count', 'Min Line Length', 'Max Line Length', 'Average Line Length')
saveRDS(object = sampleStats, file = paste0(g_corpus_directory_en, '\\en_US.filestats_partition_', sprintf('%.1f', sampleRate), '.rds'))
