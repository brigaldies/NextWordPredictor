answerQuiz <- function(model, sentence, choices) {
    answer = NULL
    ...
    quadgramsMatches = model$quadgrams[gramLookup][grepl(choicesRegex, gram)][order(logprob, decreasing = TRUE)]
    if (dim(quadgramsMatches)[1] > 0 & !is.na(quadgramsMatches[1, gram])) {
        ...
        message(paste0('Looking up "', gramLookup, '"'))
        trigramsMatches = model$trigrams[gramLookup][grepl(choicesRegex, gram)][order(logprob, decreasing = TRUE)]
        if (dim(trigramsMatches)[1] > 0 & !is.na(trigramsMatches[1, gram])) {
            ...
            message(paste0('Looking up "', gramLookup, '"'))
            bigramsMatches = model$bigrams[gramLookup][grepl(choicesRegex, gram)][order(logprob, decreasing = TRUE)]
